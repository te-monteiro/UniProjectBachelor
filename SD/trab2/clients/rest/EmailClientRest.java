...
public abstract class EmailClientRest {
	...
	// This method receives and operation (called from MessageClientRest and
	// UserClientRest), and retries it
	...
	protected <T> EmailResponse<T> tryMultiple(Supplier<EmailResponse<T>> operation) {
		...
		while (nTries < maxRetries)
			try {
				...
				nTries++;
				System.out.println("Timeout on rest client for " + target.toString() + " (" + nTries + "/" + maxRetries
						+ ") " + e.getMessage());
				try {
