...
public class MessageClientSoap extends EmailClientSoap implements MessagesEmailClient {
	...
	// These method work like in UserClientSoap, using "tryMultiple" to retry the
	...
	public EmailResponse<Void> deleteUserInfo(String user, String secret) {
		return tryMultiple(() -> {
			getClient().removeInbox(user, secret);
	...
	public EmailResponse<Void> setupUserInfo(String user, String secret) {
		return tryMultiple(() -> {
			getClient().postInbox(user, secret);
			return EmailResponse.noContent();
		});
	}

	public EmailResponse<List<String>> forwardSendMessage(Message msg, String secret) {
		return tryMultiple(() -> EmailResponse.ok(getClient().postMessageOutside(msg, secret)));
	}

	public EmailResponse<Void> forwardDeleteSentMessage(long mid, String secret) {
		return tryMultiple(() -> {
			getClient().processDelete(mid, secret);
			return EmailResponse.noContent();
		});
	}

	// this method creates a new MessageServiceSoap proxy, or returns the existing
