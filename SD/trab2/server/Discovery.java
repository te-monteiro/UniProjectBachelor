...
public class Discovery {
	...
	public URI[] knownUrisOf(String serviceName) {
		...
		List<Map.Entry<URI, Long>> sorted = new LinkedList<>(aux.entrySet());
		Collections.sort(sorted, new ComparatorByTime());
		// throw new Error("Service name not found...");
		// URI[] uris = new URI[aux.keySet().size()];
		// return aux.keySet().toArray(uris);
		URI[] uris = new URI[sorted.size()];
		for (int i = 0; i < sorted.size(); i++) {
			uris[i] = sorted.get(i).getKey();
		}
