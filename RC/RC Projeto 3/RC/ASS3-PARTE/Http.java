...
public class Http {
	...
	/**
	 * Parses an HTTP header returning an array with the name of the attribute
	 * header in position 0 and its value in position 1 Example, for "Connection:
	 ...
	 * Examples: "bytes=1000-2000" returns: { 1000, 2000 } "bytes=1000-" returns: {
	 * 1000, -1 }
	 * 
	 * If the input is malformed, it returns null
	 */
	...
	/**
	 * Parses an HTTP reply range value, returned by a server it returns an array
	 * with the range values as well as the size of the file Examples: "bytes
	 * 0-20/3000" returns { 0, 20, 3000 } "bytes 10-30" returns { 10, 30, -1 } If
	 * the input is malformed, it returns null
	 */
	...
	static final Pattern HTTP_SERVER_HEADER_RANGE_REGEX = Pattern.compile("bytes\\s(\\d+)-(\\d+)/?(\\d+|\\*)?");

	/**
	 * Sends an error message "501: Request Not Implemented"
	 */
	public static void sendsNotImplementedPage(OutputStream out) throws IOException {
		String page = "<HTML><BODY>501: Request Not Implemented</BODY></HTML>\r\n";
	 ...
	 */
	public static void sendsSimplePage(String simplePage, OutputStream out) throws IOException {
