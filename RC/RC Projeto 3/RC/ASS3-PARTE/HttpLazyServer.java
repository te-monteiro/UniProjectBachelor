...
public class HttpLazyServer {
	...
	private static void sendsNotSupportedPage(OutputStream out) throws IOException {
		String page = "<HTML><BODY>Lazy server: request not supported</BODY></HTML>";
	 ...
	 */
	private static void sendsSimplePage(String simplePage, OutputStream out) throws IOException {
		String page = "<HTML><BODY>Lazy server: " + simplePage + "</BODY></HTML>\r\n";
		int length = page.length();
	...
	private static void sendFile(String fileName, int[] ranges, OutputStream out) throws IOException {
		...
		if (name == "")
			sendsSimplePage("The empty name is not a file", out);
		else if (!f.exists())
			sendsSimplePage("File \"" + fileName + "\" does not exist", out);
		else if (!f.isFile())
			sendsSimplePage("File \"" + fileName + "\" is not a file", out);
		else if (!f.canRead())
			sendsSimplePage("File \"" + fileName + "\" cannot be read", out);
		else {
			...
			if (rest > MAX_BYTES)
				rest = MAX_BYTES; // never sends more then MAX_BYTES
			...
			// but the all file is sent (bytes=0-fileSize, bytes=0-, bytes=0-something too
			// big)
