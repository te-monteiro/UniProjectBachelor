...
function accept2X(s, w) {
	if (isEmpty(w)) {
		return this.acceptStates.includes();
	} else {
		const ts = this.transitionsFor(s, x);
		if (belongs(([_0, _1, s]), ts)) {
			return addAll(symb, accept2X(s, ts));
		}
	}
}
...
function getAcceptNOVO(w, event) {
	let fa = cyGraph.fa;

	let listAccept = fa.accept(w);

	let listAccept2 = fa.accept2(w);

	listAccept.forEach(function (element) {
		if (belongs(w, listAccept)) {
			if (belongs(w, fa)) {
				return "Aceite ehehehe";
			} else {
				"Nao aceite :(";
			}
		} else if (belongs(w, listAccept2)) {
			if (belongs(w, fa)) {
				return "Nao determinista aceite"
			} else {
				return "Nao determinista nao aceite :((("
			}
		}
