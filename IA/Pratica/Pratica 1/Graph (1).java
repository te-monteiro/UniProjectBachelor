...
public class Graph {
	...
	public Node searchSolutionNew(String initLabel, String goalLabel, String set, Algorithms algID) {
		Graph newGraph = new Graph();

		newGraph.addVertice(initLabel, this.getVertice(initLabel).getLatitude(),
				this.getVertice(initLabel).getLongitude());
		newGraph.addVertice(goalLabel, this.getVertice(goalLabel).getLatitude(),
				this.getVertice(goalLabel).getLongitude());
		VertexSet newSet = this.getVerticeSet(set);

		for (Vertex label : newSet.getVertices()) {
			newGraph.addVertice(label.getLabel(), label.getLatitude(), label.getLongitude());
			newGraph.addEdge(initLabel, label.getLabel(),
					searchSolution(initLabel, label.getLabel(), algID).getPathCost());
			newGraph.addEdge(label.getLabel(), goalLabel,
					searchSolution(label.getLabel(), goalLabel, algID).getPathCost());
		}
	...
	public Node SearchSolutionBySets(String initLabel, String goalLabel, ArrayList<String> sets, Algorithms algID) {
		Graph newGraph = new Graph();
		...
		String currentPos = it.next();
		
		while (it.hasNext()) {
			String nextPos = it.next();
			for (Vertex label1 : getVerticeSet(currentPos).getVertices()) {
				for (Vertex label2 : getVerticeSet(nextPos).getVertices())
					newGraph.addEdge(label1.getLabel(), label2.getLabel(),
							searchSolution(label1.getLabel(), label2.getLabel(), algID).getPathCost());
			}

			currentPos = nextPos;
		}

		for (Vertex label : getVerticeSet(currentPos).getVertices())
			newGraph.addEdge(label.getLabel(), goalLabel,
					searchSolution(label.getLabel(), goalLabel, algID).getPathCost());

		for (Vertex label : getVerticeSet(sets.get(0)).getVertices())
			newGraph.addEdge(initLabel, label.getLabel(),
					searchSolution(initLabel, label.getLabel(), algID).getPathCost());
